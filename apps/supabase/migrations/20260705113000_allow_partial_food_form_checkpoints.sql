alter table public.food_form_details
  alter column water_liters drop not null,
  alter column has_other_fluids drop not null;
...
alter table public.food_form_details
  ...
  check (
    (
      has_other_fluids is null
      and other_fluids is null
    )
    or (
      has_other_fluids
      and (
        other_fluids is null
        or nullif(btrim(other_fluids), '') is not null
      )
    )
    or (
      has_other_fluids is false
      and other_fluids is null
    )
  );
...
begin
  ...
  if p_has_other_fluids and p_other_fluids is not null and p_other_fluids like v_other_fluids_prefix || '%' then
    begin
