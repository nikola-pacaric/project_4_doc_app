        ...
        jsonb_strip_nulls(
          jsonb_build_object(
            ...
            'details', case entry.kind
              ...
              when 'symptom' then (
                select to_jsonb(symptom_detail)
                  - 'entry_id'
                  - 'intake_list'
...
comment on function app_private.export_patient_data(uuid, public.export_mode, public.export_range_type, date, date) is 'Exports linked patient data without obsolete symptom fields or embedded image binaries.';
